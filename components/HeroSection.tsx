  ...
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center text-white max-w-4xl mx-auto px-4">
        <h1 className="text-4xl md:text-6xl font-medium mb-4 tracking-wider uppercase">{slides[activeSlide].title}</h1>
        <p className="text-lg md:text-xl mb-8 tracking-wide">{slides[activeSlide].subtitle}</p>
        <Button
          asChild
          size="lg"
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex space-x-2">
        {slides.map((_, i) => (
          <button
            key={i}
            className={`w-3 h-3 rounded-full transition-colors ${i === activeSlide ? "bg-white" : "bg-white/50"}`}
            onClick={() => goToSlide(i)}
          />
        ))}
      </div>
