...
export async function POST(request: NextRequest) {
  try {
    ...
    images: [
      ${formattedProduct.images.map((img: string) => `"${img}"`).join(',\n      ')},
    ],
    ...
    sizes: [${formattedProduct.sizes.map((size: string) => `"${size}"`).join(', ')}],
    colors: [${formattedProduct.colors.map((color: string) => `"${color}"`).join(', ')}],
    stock: ${formattedProduct.stock},
    brand: "${formattedProduct.brand}",
    tags: [${formattedProduct.tags.map((tag: string) => `"${tag}"`).join(', ')}],
